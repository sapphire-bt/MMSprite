  ...
  class Sprite {
    ...
    frameToCanvas = function (frameIndex) {
      ...
      for (let y = 0; y < frame.height; y++) {
        ...
        const deltaLength =
          (y + 1 === frame.height
            ? frame.pixel_offsets[0]
            : frame.delta_offsets[y + 1]) - deltaOffset;
        ...
        let cursor = frame.begin_offset + pixelOffset;

        for (let i = 0; i < delta.length; i++) {
          if (i & 1) {
            for (let j = 0; j < delta[i]; j++) {
              const colour =
                this.palettes[frame.palette_index][
                  reader.dataView.getUint8(cursor++)
                ];
